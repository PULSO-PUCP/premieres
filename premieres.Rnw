...
\begin{figure}[h]
\centering
\begin{adjustbox}{clip,trim=0cm 0.5cm 0cm 0cm} 
<<lolliDURpcm, echo=FALSE, fig=TRUE>>=
...
df <- arrow::read_parquet("spellsModel.parquet") %>%
  arrange(start) %>%
  mutate(
...
# Define "max" whisker (upper bound for typical values)
...
# Begin plot
ggplot(df, aes(x = spell_duration, y = spell_label)) +
...
geom_vline(xintercept = max_whisker, linetype = "dotted", color = "red")+
  labs(
    title = "Duration of Prime Ministers in Office (Peru, 1980–2025)",
    x = "Days in Office",
    y = NULL
  ) +
  theme_minimal() +
...
@
\end{adjustbox}
\caption{Duration of Prime Ministers from 1980 until 2025}  
\label{lolliDURpcm} 
\end{figure}
...
# linkGit2='/raw/refs/heads/main/primesmodel.parquet'
# linkPrimes=paste0(linkGit1,linkGit2)
data=arrow::read_parquet('primesmodel.parquet')
namesHeaders=c("Era","count","mean","min",'max','range')
summary_data <- data %>%
  group_by(era) %>%
  summarise(
    N = n(),
    Mean_Var1 = mean(spell_duration),
    Min = min(spell_duration),
    Max = max(spell_duration),
    Range=max(spell_duration)-min(spell_duration)
  ) %>%kbl(format='latex',
           booktabs = T,
           digits = 2,                #label cross-ref!!
           caption = "Duration in Days by era\\label{tab:durationPerEra}",
           col.names = namesHeaders)%>%
    kable_classic_2(latex_options = "h") %>%footnote(general = "author calculations from official appointment resolutions and El Peruano archives.",footnote_as_chunk = T,
general_title = "Source: ",title_format = c("italic"),threeparttable = T
)
summary_data
@
